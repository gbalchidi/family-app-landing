...
export default function BillingForm({ userData, selectedPlan, onComplete, onBack }: BillingFormProps) {
  ...
  const getPlanPrice = (plan: Plan): string => {
    switch (plan) {
      ...
      case 'family': return '499₽'
      case 'premium': return '699₽'
    }
  }

  const getPlanDescription = (plan: Plan): string => {
    switch (plan) {
      case 'basic': return 'Базовый тариф'
      case 'family': return 'Семейный тариф'
      case 'premium': return 'Премиум тариф'
    }
  }
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <motion.button
          ...
          className="w-full bg-gradient-to-r from-primary to-secondary text-white font-semibold py-4 px-4 rounded-2xl hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isProcessing ? (
            ...
            <div className="flex items-center justify-center gap-2 w-full">
              ...
              <span className="block sm:inline text-sm sm:text-base">
                <span className="sm:hidden">Начать пробный период</span>
                <span className="hidden sm:inline">Начать 14-дневный пробный период</span>
              </span>
            </div>
          )}
        </motion.button>
